...
import { Color, CssBaseline } from '@material-ui/core';
...
const App: FC = (): ReactElement => {
    ...
    useEffect(() => {
        ...
        stompClient.connect({}, () => {
            stompClient?.subscribe('/topic/feed', (res) => {
                dispatch(setUpdatedTweet(JSON.parse(res.body)));
                dispatch(updateNotificationInfoTweet(JSON.parse(res.body)));
            });

            stompClient?.subscribe('/topic/feed/add', (res) => {
                dispatch(setTweet(JSON.parse(res.body)));
            });

            stompClient?.subscribe('/topic/feed/schedule', (res) => {
                dispatch(setScheduledTweets(JSON.parse(res.body)));
            });
        });
        ...
        if (myProfileId) {
            ...
            stompClient.connect({}, () => {
                ...
                stompClient?.subscribe(
                    `/topic/notifications/${myProfileId}`,
                    (response) => {
            ...
            <div className='App'>
                ...
                >
                    <Switch location={background || location}>
                        ...
                        <Route
                            path={`${PROFILE}/:userId${TOPICS}`}
                            component={UserTopics}
                            exact
                        />
                        <Route
                            path={`${USER_FOLLOWERS_YOU_FOLLOW}/:id`}
                    ...
                    {background && (
                        <Route
                            path={`${PROFILE_PHOTO}/:id`}
                            children={<UserImageModal />}
                        />
                    )}
                    {background && (
                        <Route
                            path={`${PROFILE_HEADER_PHOTO}/:id`}
                            children={<UserImageModal />}
                        />
                    )}
                </Layout>
